...
class SynthVLAPolicy(InferencePolicy, StatefulPolicy):
    ...
    def _populate_action_buffer(self, observation) -> None:
        ...
        # Call agent
        logger.debug(f"[Step {self.step_count}] Calling agent for action chunk prediction...")
        pred_actions = self.agent.get_action_chunk(
            images=images,
            task_description=goal,
            state=state,
        )
        logger.debug(f"[Step {self.step_count}] Predicted action chunk: shape={pred_actions.shape}")
        ...
        self.buffer_index = 0
        logger.debug(f"[Step {self.step_count}] Populated action buffer with {len(self.action_buffer)} actions")
    ...
    def inference_model(self, model_input) -> dict[str, np.ndarray]:
        ...
        logger.debug(f"[Step {self.step_count}] Executing action {self.buffer_index + 1}/{len(self.action_buffer)} (buffer will refresh at {self.execute_horizon} steps)")
        ...
        if self.step_count % 50 == 0:
            logger.debug(f"[PROGRESS] Completed {self.step_count} steps")
