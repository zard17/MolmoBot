...
Requires a benchmark directory with `benchmark.json` (generated by `run_batch_eval.py --generate-config` or upstream tooling).

```bash
# Linux
python run_eval_with_glfw.py \
    --checkpoint_path <path> \
    --benchmark_path <path_to_benchmark_dir> \
