...
# -- Patch molmo_spaces to use HF when MLSPACES_USE_HF=1 (R2 blocked in some envs) --
import molmo_spaces.molmo_spaces_constants as _msc
if os.environ.get("MLSPACES_USE_HF", "0") == "1":
...
def get_default_config():
    """Return default batch evaluation config dict with 4 groups for comparison."""
...
def generate_default_config():
    """Generate default config, write to disk, and print summary."""
    ...
    total = len(config["tasks"]) * config["repeats"]
...
def main():
    ...
    parser.add_argument("--config", type=str, help="Path to batch config JSON (uses built-in defaults if omitted)")
    ...
    if args.config:
        ...
        print("No --config provided, using built-in default configuration")
        config = get_default_config()
